...
maxSizeToCount = 100000

nodesSizes = []
traverseTree(n -> isDir(n) && push!(nodesSizes, calculateDirSize(n)), rootNode)
nodesSizesFiltered = filter(s -> s<=maxSizeToCount, nodesSizes)
#totalSize = calculateDirSize(rootNode)
println("nodesSizes ", nodesSizes)
println("nodesSizesFiltered ", nodesSizesFiltered)
println("sum nodesSizesFiltered ", sum(nodesSizesFiltered))